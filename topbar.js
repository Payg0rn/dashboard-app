...
// pushes a merged update to the Supabase water row so the
...
(function () {
  ...
  const TOPBAR_SUPABASE_URL = 'https://rahjrdglaetvkrgnimjw.supabase.co';
  const TOPBAR_SUPABASE_KEY = 'sb_publishable_CharrrSHen069fNtMmbu9w_U4nBJNg-';
...
`;
  ...
  // -------- HTML --------
  // All links point to index.html since the entire app is a single-page SPA.
  const html = `
<header class="topbar" id="topbar" role="navigation" aria-label="Quick stats">
  ...
  <a href="index.html" class="topbar-pill" id="topbarStack">
    <span class="topbar-pill-dot"></span>
    <span class="topbar-pill-label">STACK</span>
    <span class="topbar-pill-count" id="topbarStackCount">-/-</span>
  </a>
  <div class="topbar-water-wrap">
    <a href="index.html" class="topbar-water-pill" id="topbarWater">
      <span class="topbar-pill-dot"></span>
...
`;
  ...
  async function pushWaterToSupabase(localWater) {
    // Skip when the page already has its own water sync (index.html).
    // The patched localStorage.setItem will push automatically.
    if (window.waterRerender) return;
    ...
    try {
      ...
      // Read the current 'water' row, merge in the updated po_water_v1 value,
      // and push back. The value is stored as a JSON string (same as localStorage).
      const { data } = await supa
        .from('app_state').select('data').eq('key', 'water').maybeSingle();
      const current = (data && data.data) || {};
      const merged = Object.assign({}, current, { po_water_v1: JSON.stringify(localWater) });
      await supa.from('app_state').upsert(
        { key: 'water', data: merged, updated_at: new Date().toISOString() },
        { onConflict: 'key' }
      );
    } catch (e) { /* offline — local change will sync next time user visits */ }
  }
